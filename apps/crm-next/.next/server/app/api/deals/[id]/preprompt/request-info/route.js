"use strict";(()=>{var e={};e.id=7604,e.ids=[7604],e.modules={3524:e=>{e.exports=require("@prisma/client")},399:e=>{e.exports=require("next/dist/compiled/next-server/app-page.runtime.prod.js")},517:e=>{e.exports=require("next/dist/compiled/next-server/app-route.runtime.prod.js")},5315:e=>{e.exports=require("path")},6605:(e,t,a)=>{a.r(t),a.d(t,{originalPathname:()=>A,patchFetch:()=>R,requestAsyncStorage:()=>T,routeModule:()=>u,serverHooks:()=>N,staticGenerationAsyncStorage:()=>m});var i={};a.r(i),a.d(i,{POST:()=>p});var r=a(3278),d=a(5002),o=a(4877),s=a(1309),n=a(7392),l=a(9004),_=a(1043),c=a(4738);async function E(e){return(await c._.$queryRaw`
    ...
    LIMIT 1
  `)[0]||null}async function p(e,{params:t}){let a=(0,n.I)(e);if(a)return a;await (0,_.CQ)();let i=await e.json().catch(()=>({})),r=String(i.projectId||i.project_id||"").trim(),d=String(i.subject||"[KoddaHub] Precisamos de mais informa\xe7\xf5es do briefing").trim(),o=String(i.message||i.notes||"").trim(),p=String(i.dueAt||"").trim(),u=p?new Date(p):null,T=(Array.isArray(i.requestItems)?i.requestItems:String(i.requestItems||"").split("\n").map(e=>e.trim()).filter(Boolean)).map(e=>String(e||"").trim()).filter(Boolean).slice(0,20);if(!o&&0===T.length)return s.NextResponse.json({error:"Informe a mensagem ou pelo menos 1 item solicitado."},{status:422});if(!r)return s.NextResponse.json({error:"projectId \xe9 obrigat\xf3rio."},{status:422});try{let e=await c._.$transaction(async e=>{let a=await e.deal.findUnique({where:{id:t.id},include:{organization:{select:{id:!0,legalName:!0,billingEmail:!0}}}});if(!a)throw Error("Deal n\xe3o encontrado");if("HOSPEDAGEM"!==a.dealType)throw Error("Fluxo pr\xe9-prompt dispon\xedvel somente para hospedagem");if("CLIENT"!==a.lifecycleStatus)throw Error("Fluxo pr\xe9-prompt dispon\xedvel apenas para cliente fechado");let s=await e.$queryRaw`
        SELECT id::text
        FROM client.projects
        WHERE id = ${r}::uuid
          AND organization_id = ${a.organizationId}::uuid
        LIMIT 1
      `;if(!s[0]?.id)throw Error("Projeto inv\xe1lido para este cliente.");let n=await e.dealPromptRevision.findFirst({where:{dealId:a.id},orderBy:{version:"desc"}}),_=n?.promptText||"",c=n?.promptJson||null;if(!_&&a.organizationId){let e=await E(a.organizationId);_=String(e?.prompt_text||""),c=e?.prompt_json||null}let p=n?n.version:1,m="string"==typeof i.promptText?i.promptText.trim():"",N=i.promptJson??null;m&&(_=m),N&&(c=N);let A=n?await e.dealPromptRevision.update({where:{id:n.id},data:{promptText:_||"Prompt pendente de refinamento.",promptJson:c,status:"REQUESTED_INFO",requestedNotes:o||T.join(" | ")||null,updatedAt:new Date}}):await e.dealPromptRevision.create({data:{dealId:a.id,version:p,promptText:_||"Prompt pendente de refinamento.",promptJson:c,status:"REQUESTED_INFO",requestedNotes:o||T.join(" | ")||null,createdBy:"ADMIN"}}),R=a.contactEmail||a.organization?.billingEmail,I=null;R&&(I=(await e.emailQueue.create({data:{organizationId:a.organizationId||null,emailTo:R,subject:d||"[KoddaHub] Precisamos de mais informa\xe7\xf5es para seu site",body:["Ol\xe1!","","Para avan\xe7armos na etapa Pr\xe9-prompt do seu Site 24h, precisamos destes detalhes:","",...T.length>0?T.map((e,t)=>`${t+1}. ${e}`):[],...o?["",o]:[],"","Responda este e-mail com as informa\xe7\xf5es solicitadas.","Equipe KoddaHub."].join("\n"),status:"PENDING"}})).id);let L=await e.$queryRaw`
        ...
        VALUES(
          ...
          ${r}::uuid,
          ${A.id}::uuid,
          ${d||"[KoddaHub] Solicita\xe7\xe3o de informa\xe7\xf5es adicionais"},
          ${JSON.stringify(T)}::jsonb,
          ${o||T.join("\n")||"Solicita\xe7\xe3o de informa\xe7\xf5es adicionais"},
          ${u&&!Number.isNaN(u.getTime())?u:null},
          ${I}::uuid,
          'SENT',
          'ADMIN',
          now(),
  ...
  `),n=!0}}async function p(){return await E(),(await d._.$queryRaw`
  ...
  `;return n[0]?.id||null}async function T(e){await E();let t=String(e||"").trim().toLowerCase(),a=t?await d._.$queryRaw`
      ...
      `:await d._.$queryRaw`
        ...
        LIMIT 1
      `;return a[0]?{id:a[0].id,code:a[0].code,name:a[0].name,rootPath:a[0].root_path,entryFile:a[0].entry_file,isDefault:a[0].is_default}:null}async function m(e,t){await E();let a=String(t||"").trim();for(let t of s)await d._.$executeRaw`
    ...
    `}async function N(e,t){await E();let a=String(t||"").trim();return d._.$queryRaw`
    SELECT
      id::text,
      deal_id::text,
      stage_code,
