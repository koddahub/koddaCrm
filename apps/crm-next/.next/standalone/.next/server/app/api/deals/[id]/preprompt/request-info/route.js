"use strict";(()=>{var e={};e.id=7604,e.ids=[7604],e.modules={3524:e=>{e.exports=require("@prisma/client")},399:e=>{e.exports=require("next/dist/compiled/next-server/app-page.runtime.prod.js")},517:e=>{e.exports=require("next/dist/compiled/next-server/app-route.runtime.prod.js")},5315:e=>{e.exports=require("path")},4897:(e,t,a)=>{a.r(t),a.d(t,{originalPathname:()=>f,patchFetch:()=>I,requestAsyncStorage:()=>E,routeModule:()=>m,serverHooks:()=>N,staticGenerationAsyncStorage:()=>T});var i={};a.r(i),a.d(i,{POST:()=>_});var o=a(3278),r=a(5002),d=a(4877),n=a(1309),s=a(7392),l=a(9004),u=a(1043),c=a(4738);async function p(e){return(await c._.$queryRaw`
    ...
    LIMIT 1
  `)[0]||null}async function _(e,{params:t}){let a=(0,s.I)(e);if(a)return a;await (0,u.CQ)();let i=await e.json().catch(()=>({})),o=String(i.subject||"[KoddaHub] Precisamos de mais informa\xe7\xf5es do briefing").trim(),r=String(i.message||i.notes||"").trim(),d=String(i.dueAt||"").trim(),_=d?new Date(d):null,m=(Array.isArray(i.requestItems)?i.requestItems:String(i.requestItems||"").split("\n").map(e=>e.trim()).filter(Boolean)).map(e=>String(e||"").trim()).filter(Boolean).slice(0,20);if(!r&&0===m.length)return n.NextResponse.json({error:"Informe a mensagem ou pelo menos 1 item solicitado."},{status:422});try{let e=await c._.$transaction(async e=>{let a=await e.deal.findUnique({where:{id:t.id},include:{organization:{select:{id:!0,legalName:!0,billingEmail:!0}}}});if(!a)throw Error("Deal n\xe3o encontrado");if("HOSPEDAGEM"!==a.dealType)throw Error("Fluxo pr\xe9-prompt dispon\xedvel somente para hospedagem");if("CLIENT"!==a.lifecycleStatus)throw Error("Fluxo pr\xe9-prompt dispon\xedvel apenas para cliente fechado");let d=await e.dealPromptRevision.findFirst({where:{dealId:a.id},orderBy:{version:"desc"}}),n=d?.promptText||"",s=d?.promptJson||null;if(!n&&a.organizationId){let e=await p(a.organizationId);n=String(e?.prompt_text||""),s=e?.prompt_json||null}let u=d?d.version:1,c="string"==typeof i.promptText?i.promptText.trim():"",E=i.promptJson??null;c&&(n=c),E&&(s=E);let T=d?await e.dealPromptRevision.update({where:{id:d.id},data:{promptText:n||"Prompt pendente de refinamento.",promptJson:s,status:"REQUESTED_INFO",requestedNotes:r||m.join(" | ")||null,updatedAt:new Date}}):await e.dealPromptRevision.create({data:{dealId:a.id,version:u,promptText:n||"Prompt pendente de refinamento.",promptJson:s,status:"REQUESTED_INFO",requestedNotes:r||m.join(" | ")||null,createdBy:"ADMIN"}}),N=a.contactEmail||a.organization?.billingEmail,f=null;N&&(f=(await e.emailQueue.create({data:{organizationId:a.organizationId||null,emailTo:N,subject:o||"[KoddaHub] Precisamos de mais informa\xe7\xf5es para seu site",body:["Ol\xe1!","","Para avan\xe7armos na etapa Pr\xe9-prompt do seu Site 24h, precisamos destes detalhes:","",...m.length>0?m.map((e,t)=>`${t+1}. ${e}`):[],...r?["",r]:[],"","Responda este e-mail com as informa\xe7\xf5es solicitadas.","Equipe KoddaHub."].join("\n"),status:"PENDING"}})).id);let I=await e.$queryRaw`
        ...
        VALUES(
          ${a.id}::uuid,
          ${T.id}::uuid,
          ${o||"[KoddaHub] Solicita\xe7\xe3o de informa\xe7\xf5es adicionais"},
          ${JSON.stringify(m)}::jsonb,
          ${r||m.join("\n")||"Solicita\xe7\xe3o de informa\xe7\xf5es adicionais"},
          ${_&&!Number.isNaN(_.getTime())?_:null},
          ${f}::uuid,
          'SENT',
          'ADMIN',
          now(),
  ...
  `),l)){let t=u(o().resolve(d,e.folder));await r._.$queryRaw`
  ...
  `;return s[0]?.id||null}async function E(e){await p();let t=String(e||"").trim().toLowerCase(),a=t?await r._.$queryRaw`
    ...
    `}async function N(e){return await p(),r._.$queryRaw`
    SELECT
      id::text,
      deal_id::text,
      stage_code,
  ...
  `}async function f(e){await p();let t=(await r._.$queryRaw`
    SELECT
    ...
    WHERE deal_id = ${e}::uuid
      AND stage_code = 'publicacao'
  `)[0]||{required_total:0,required_completed:0,pending_total:0},a=Number(t.required_total||0),i=Number(t.required_completed||0),o=Number(t.pending_total||0);return{requiredTotal:a,requiredCompleted:i,pendingTotal:o,ready:a>0&&0===o}}}};var t=require("../../../../../../webpack-runtime.js");t.C(e);var a=e=>t(t.s=e),i=t.X(0,[7787,4833],()=>a(4897));module.exports=i})();