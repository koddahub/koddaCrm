"use strict";(()=>{var e={};e.id=7604,e.ids=[7604],e.modules={3524:e=>{e.exports=require("@prisma/client")},399:e=>{e.exports=require("next/dist/compiled/next-server/app-page.runtime.prod.js")},517:e=>{e.exports=require("next/dist/compiled/next-server/app-route.runtime.prod.js")},5315:e=>{e.exports=require("path")},4897:(e,t,a)=>{a.r(t),a.d(t,{originalPathname:()=>A,patchFetch:()=>R,requestAsyncStorage:()=>m,routeModule:()=>E,serverHooks:()=>N,staticGenerationAsyncStorage:()=>T});var i={};a.r(i),a.d(i,{POST:()=>p});var r=a(3278),d=a(5002),o=a(4877),s=a(1309),n=a(7392),l=a(9004),_=a(1043),u=a(4738);async function c(e){return(await u._.$queryRaw`
    ...
    LIMIT 1
  `)[0]||null}async function p(e,{params:t}){let a=(0,n.I)(e);if(a)return a;await (0,_.CQ)();let i=await e.json().catch(()=>({})),r=String(i.subject||"[KoddaHub] Precisamos de mais informa\xe7\xf5es do briefing").trim(),d=String(i.message||i.notes||"").trim(),o=String(i.dueAt||"").trim(),p=o?new Date(o):null,E=(Array.isArray(i.requestItems)?i.requestItems:String(i.requestItems||"").split("\n").map(e=>e.trim()).filter(Boolean)).map(e=>String(e||"").trim()).filter(Boolean).slice(0,20);if(!d&&0===E.length)return s.NextResponse.json({error:"Informe a mensagem ou pelo menos 1 item solicitado."},{status:422});try{let e=await u._.$transaction(async e=>{let a=await e.deal.findUnique({where:{id:t.id},include:{organization:{select:{id:!0,legalName:!0,billingEmail:!0}}}});if(!a)throw Error("Deal n\xe3o encontrado");if("HOSPEDAGEM"!==a.dealType)throw Error("Fluxo pr\xe9-prompt dispon\xedvel somente para hospedagem");if("CLIENT"!==a.lifecycleStatus)throw Error("Fluxo pr\xe9-prompt dispon\xedvel apenas para cliente fechado");let o=await e.dealPromptRevision.findFirst({where:{dealId:a.id},orderBy:{version:"desc"}}),s=o?.promptText||"",n=o?.promptJson||null;if(!s&&a.organizationId){let e=await c(a.organizationId);s=String(e?.prompt_text||""),n=e?.prompt_json||null}let _=o?o.version:1,u="string"==typeof i.promptText?i.promptText.trim():"",m=i.promptJson??null;u&&(s=u),m&&(n=m);let T=o?await e.dealPromptRevision.update({where:{id:o.id},data:{promptText:s||"Prompt pendente de refinamento.",promptJson:n,status:"REQUESTED_INFO",requestedNotes:d||E.join(" | ")||null,updatedAt:new Date}}):await e.dealPromptRevision.create({data:{dealId:a.id,version:_,promptText:s||"Prompt pendente de refinamento.",promptJson:n,status:"REQUESTED_INFO",requestedNotes:d||E.join(" | ")||null,createdBy:"ADMIN"}}),N=a.contactEmail||a.organization?.billingEmail,A=null;N&&(A=(await e.emailQueue.create({data:{organizationId:a.organizationId||null,emailTo:N,subject:r||"[KoddaHub] Precisamos de mais informa\xe7\xf5es para seu site",body:["Ol\xe1!","","Para avan\xe7armos na etapa Pr\xe9-prompt do seu Site 24h, precisamos destes detalhes:","",...E.length>0?E.map((e,t)=>`${t+1}. ${e}`):[],...d?["",d]:[],"","Responda este e-mail com as informa\xe7\xf5es solicitadas.","Equipe KoddaHub."].join("\n"),status:"PENDING"}})).id);let R=await e.$queryRaw`
  ...
  `),n=!0}}async function p(){return await c(),(await d._.$queryRaw`
  ...
  `).map(e=>({id:e.id,code:e.code,name:e.name,rootPath:e.root_path,entryFile:e.entry_file,isDefault:e.is_default,isActive:e.is_active,createdAt:e.created_at,updatedAt:e.updated_at}))}async function E(e){await c();let t=e.code.trim().toLowerCase().replace(/[^a-z0-9_-]/g,"_");if(!t)throw Error("C\xf3digo do modelo inv\xe1lido");let a=e.name.trim();if(!a)throw Error("Nome do modelo \xe9 obrigat\xf3rio");let i=_(e.rootPath),r=(e.entryFile||"index.html").replace(/^\/+/,"").trim()||"index.html",o=!!e.isDefault,s=void 0===e.isActive||!!e.isActive;o&&await d._.$executeRaw`UPDATE crm.template_model_catalog SET is_default=false, updated_at=now()`;let n=await d._.$queryRaw`
    ...
    DO UPDATE SET
      ...
      updated_at = now()
    RETURNING id::text
  `;return n[0]?.id||null}async function m(e){await c();let t=String(e||"").trim().toLowerCase(),a=t?await d._.$queryRaw`
      ...
      `:await d._.$queryRaw`
        ...
        LIMIT 1
      `;return a[0]?{id:a[0].id,code:a[0].code,name:a[0].name,rootPath:a[0].root_path,entryFile:a[0].entry_file,isDefault:a[0].is_default}:null}async function T(e){for(let t of(await c(),s))await d._.$executeRaw`
    ...
    `}async function N(e){return await c(),d._.$queryRaw`
    SELECT
      id::text,
      deal_id::text,
      stage_code,
  ...
  `}async function A(e){await c();let t=(await d._.$queryRaw`
    SELECT
